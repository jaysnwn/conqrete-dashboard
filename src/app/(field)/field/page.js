...
export default function FieldPortal() {
  ...
  const fetchLoginData = async () => {
    ...
    if (rep) {
      // It's a Salesman! Auto log them in
      loginAsRep(rep);
    } else {
      // It's the Admin! Show the full list so Admin can test
  ...
  // NEW: Log Out Function updated to redirect
  const handleLogOut = async () => {
    await supabase.auth.signOut();
    router.push("/login"); // 3. Added the redirect!
  };
  ...
  const handleDeletePunch = async (orderId, orderNum, totalAmount, customerName) => {
    ...
    try {
      // 1. Fetch the specific items from this order so we know what to put back
      const { data: items } = await supabase.from("order_items").select("*").eq("order_id", orderId);

      // 2. Put the inventory back on the shelves!
      ...
      // 3. Fix the Retailer's Financial Ledger
      ...
      if (retailer) {
        ...
        await supabase.from("retailers").update({ 
          total_lifetime_sales: newLifetime, 
          total_pending: newPending 
        }).eq("id", retailer.id);
      }

      // 4. Finally, destroy the order records
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-32">
      ...
      {/* NEW: DUAL ACTION BOTTOM BAR */}
      <div className="fixed bottom-6 right-4 left-4 flex gap-3 z-20">
        <button 
          onClick={() => setIsCatalogOpen(true)} 
          className="flex-[1] bg-[#111] border border-gray-700 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-2xl active:scale-95 transition-transform flex items-center justify-center gap-2"
        >
          <span className="text-lg">📦</span> Catalog
        </button>
        <button 
          onClick={() => setIsCreatingOrder(true)} 
          className="flex-[2] bg-cyan-400 text-black py-4 rounded-2xl font-black uppercase tracking-widest text-sm shadow-[0_10px_40px_rgba(34,211,238,0.4)] active:scale-95 transition-transform"
        >
          + Punch Order
        </button>
      </div>

      {/* NEW: FULL SCREEN PRODUCT CATALOG MODAL */}
      {isCatalogOpen && (
        <div className="fixed inset-0 bg-[#050505] z-[80] overflow-y-auto pb-20 animate-slide-up">
          ...
          <div className="p-4 md:p-6">
            <input 
              ...
              className="w-full bg-[#111] border border-gray-800 p-4 rounded-2xl text-white text-sm outline-none focus:border-cyan-400 mb-6"
            />

            <div className="space-y-4">
              {filteredCatalog.map(product => (
                <div key={product.id} className="bg-[#0a0a0a] border border-gray-800 p-5 rounded-3xl shadow-lg flex gap-5 items-center">
                  <div className="w-16 h-16 bg-[#111] border border-gray-800 rounded-2xl flex items-center justify-center text-3xl shrink-0">
                    {product.image}
                  </div>
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      <h3 className="text-white font-black text-sm uppercase tracking-tight">{product.name}</h3>
                      <span className={`text-[8px] uppercase font-black px-2 py-1 rounded border ${product.stock > 0 ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/10' : 'text-red-400 border-red-500/20 bg-red-500/10'}`}>
                        {product.stock > 0 ? 'In Stock' : 'Out of Stock'}
                      </span>
                    </div>
                    <p className="text-[10px] text-gray-500 font-mono mb-3">{product.sku}</p>
                    
                    {/* PRICING ONLY - COST IS STRICTLY HIDDEN */}
                    <div className="grid grid-cols-2 gap-2 mt-2 pt-3 border-t border-gray-800/50">
                      <div>
                        <p className="text-[8px] text-gray-500 uppercase tracking-widest font-bold mb-1">Retailer Price</p>
                        <p className="text-white font-mono font-bold text-sm">₹{Number(product.pricing?.retailer || 0).toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-[8px] text-gray-500 uppercase tracking-widest font-bold mb-1">Distributor Price</p>
                        <p className="text-cyan-400 font-mono font-bold text-sm">₹{Number(product.pricing?.distributor || 0).toLocaleString()}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ))}
              {filteredCatalog.length === 0 && <p className="text-center text-xs text-gray-600 font-mono py-10">No products found.</p>}
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-[#0a0a0a] border border-gray-800 p-6 rounded-t-3xl sm:rounded-3xl w-full shadow-2xl animate-slide-up">
            ...
            <form onSubmit={handleAddNewShop} className="space-y-4">
              <div>
                ...
                <input required autoFocus type="text" value={newShop.store_name} onChange={e => setNewShop({...newShop, store_name: e.target.value})} className="w-full bg-[#111] border border-gray-800 p-4 rounded-xl text-white text-sm outline-none focus:border-cyan-400" placeholder="e.g. Mobile Hub" />
              </div>
              <div>
                <label className="block text-[10px] text-gray-500 mb-2 uppercase font-black">Location</label>
                <input required type="text" value={newShop.location} onChange={e => setNewShop({...newShop, location: e.target.value})} className="w-full bg-[#111] border border-gray-800 p-4 rounded-xl text-white text-sm outline-none focus:border-cyan-400" placeholder="e.g. Main Market" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-gray-500 mb-2 uppercase font-black">Phone</label>
                  <input type="tel" value={newShop.phone} onChange={e => setNewShop({...newShop, phone: e.target.value})} className="w-full bg-[#111] border border-gray-800 p-4 rounded-xl text-white text-sm outline-none focus:border-cyan-400 font-mono" placeholder="9922..." />
                </div>
                <div>
                  <label className="block text-[10px] text-gray-500 mb-2 uppercase font-black">Email</label>
                  <input type="email" value={newShop.email} onChange={e => setNewShop({...newShop, email: e.target.value})} className="w-full bg-[#111] border border-gray-800 p-4 rounded-xl text-white text-sm outline-none focus:border-cyan-400" placeholder="shop@email.com" />
                </div>
              </div>
          ...
          <div className="p-4 pt-8 md:p-6">
            ...
            <form onSubmit={submitMobileOrder} className="space-y-6 pb-32">
              ...
              <div className="pt-4 mt-6 border-t border-gray-800">
                <p className="text-[10px] text-cyan-400 mb-4 uppercase font-black tracking-widest pl-2">Add Products (Live Inventory)</p>
                {cart.map((item, index) => (
                  <div key={index} className="bg-[#111] p-5 rounded-2xl border border-gray-800 mb-4 relative shadow-lg">
                    <select required value={item.productId} onChange={e => updateCartItem(index, 'productId', e.target.value)} className="w-full bg-[#0a0a0a] text-white p-4 rounded-xl text-sm outline-none mb-4 border border-gray-700 focus:border-cyan-400">
                      ...
                      {products.map(p => (
                        <option key={p.id} value={p.id} disabled={p.stock <= 0} className="bg-[#0a0a0a] text-white">
                    ...
                    <div className="flex justify-between items-center bg-[#0a0a0a] p-3 rounded-xl border border-gray-800">
                      <div className="flex items-center gap-4">
                        ...
                        <input type="number" min="1" value={item.quantity} onChange={e => updateCartItem(index, 'quantity', e.target.value)} className="w-16 bg-[#111] border border-gray-700 py-2 px-1 rounded-lg text-center text-white font-mono text-base outline-none focus:border-cyan-400" />
                      </div>
                ...
                <button type="button" onClick={handleAddItem} className="w-full border-2 border-dashed border-gray-800 hover:border-cyan-900 text-gray-400 hover:text-cyan-400 py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest mb-10 transition-colors">
                  + Add Another Product
                </button>
              </div>
              ...
              <div className="fixed bottom-0 left-0 w-full bg-[#0a0a0a]/95 backdrop-blur-md border-t border-gray-800 p-5 pb-8 flex justify-between items-center z-40 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
                <div>
                  ...
                  <p className="text-2xl font-mono text-white font-black tracking-tighter">₹{calculateTotal().toLocaleString()}</p>
                </div>
                <button type="submit" className="bg-cyan-400 text-black px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-[0_0_20px_rgba(34,211,238,0.3)] active:scale-95 transition-transform">
                  Punch Order
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 bg-black/95 flex items-end justify-center sm:items-center sm:p-6 z-[70] backdrop-blur-xl">
          <div className="bg-[#0a0a0a] border border-gray-800 p-6 rounded-t-3xl sm:rounded-3xl w-full max-w-sm shadow-2xl animate-slide-up">
            <div className="flex justify-between items-center mb-6 border-b border-gray-800 pb-4">
              <h2 className="text-lg font-black text-white uppercase tracking-widest">Receive Cash</h2>
              <button onClick={() => setIsPaymentModalOpen(false)} className="text-gray-500 hover:text-white text-xl bg-gray-900 w-8 h-8 rounded-full flex items-center justify-center">×</button>
            </div>
            
            <form onSubmit={handleLogPayment} className="space-y-6">
              <div>
                <label className="block text-[10px] text-gray-500 mb-2 uppercase font-black tracking-widest text-center">Amount Received (₹)</label>
                <input 
                  type="number" 
                  autoFocus
                  required 
                  ...
                  className="w-full bg-[#111] border border-gray-800 py-6 px-4 rounded-2xl text-emerald-400 text-4xl font-mono font-black outline-none focus:border-emerald-500 text-center"
                />
                <p className="text-[10px] text-gray-500 mt-3 font-mono text-center">Remaining Due: ₹{(Number(selectedOrder.total_amount) - Number(selectedOrder.amount_paid || 0)).toLocaleString()}</p>
              </div>

              <button type="submit" className="w-full bg-emerald-500 text-black py-5 rounded-2xl font-black uppercase text-sm tracking-widest active:scale-95 transition-transform shadow-[0_10px_30px_rgba(16,185,129,0.3)]">
                Log Payment
              </button>
            </form>
          </div>
